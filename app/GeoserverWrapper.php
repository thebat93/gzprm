...
class GeoserverWrapper extends Model {
	var $serverUrl = '';
	var $username = '';
	var $password = '';
...
// Workspace APIs
	public function listWorkspaces() {
        ...
        public function listAllLayers() {
                return json_decode($this->runApi('layers.json'));
        }
...
// Layer APIs
	...
	public function createLayer($layerName, $workspaceName, $datastoreName, $description = '') {
		...
		public function createLayerSRS($layerName, $workspaceName, $SRS, $datastoreName) {
		// Add the store's feature type:
		// If layerName is a shapefile, the shapefile should exist in store already; uploaded via external means
		// If layerName is a postgis database table, that table should already exist
		//zsumo:manualtable
		return $this->runApi('workspaces/'.urlencode($workspaceName).'/datastores/'.urlencode($datastoreName).'/featuretypes.xml', 'POST', 
		'<featureType>
			<name>'.$layerName.'</name>
			...
			<store class="dataStore"><name>users:users</name></store>
		</featureType>');
	}
...
// Style APIs
	...
	public function addStyleToLayer($layerName, $workspaceName, $styleName) {
		...
		return $this->runApi('layers/users:'.urlencode($layerName), 'PUT', '<layer><defaultStyle><name>polygon</name></defaultStyle></layer>');
	}
