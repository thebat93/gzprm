...
//Снимок    
      ...
      var geotiff = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({//geoTiffSrc
          url: 'http://192.168.255.140:8080/geoserver/users/wms',
          params: {'LAYERS': 'users:NPP_21616_151230010535'}
        // new ol.source.ImageWMS({
        //   url: 'http://192.168.255.140:8080/geoserver/users/wms',
        //   params: tiffs//{LAYERS: 'users:NPP_21616_151230010535'}//'users:NPP_21616_151230010535, users:MEA_46181_150913090313_1701n00'}//tiffs//{LAYERS: tiffs}//'users:NPP_21616_151230010535, users:MEA_46181_150913090313_1701n00'}
         })
      });
var datas;
      ...
      geotiff.getSource().updateParams(
        //parameters['LAYERS']
        {'LAYERS': data}//'users:MEA_46181_150913090313_1701n00'}
        );
        });
    //});
        //var parameters = {'LAYERS': datas};
      //alert(parameters.LAYERS);

    //alert('123'+parameters['LAYERS']);
      //alert(geotiff.getProperties().source.i.LAYERS);//.params.LAYERS);
//////////////////////////////////////////////////////////
...
//Слой пользователя с заказами
        var filterstr="{{ $user_id }}"; //фильтр по ID
        var user_source = new ol.source.Vector({
            url:"http://192.168.255.140:8080/geoserver/ows?service=WFS&request=GetFeature&version=1.1.0&typeName=users:orders&Filter=<Filter><PropertyIsEqualTo><PropertyName>user_id</PropertyName><Literal>"+filterstr+"</Literal></PropertyIsEqualTo></Filter>&outputFormat=json",				
...
//////////////////////////////////////////////////////////
//Определение объектов контейнера
...
//                        } 
      ...
      ////////////////////////////////////////////////////
      //Настройка контролов для карты и ее активация
...
//        }),
            ...
            // }),
            geotiff, 
            archive_layer, user_layer, vector],
        overlays: [overlay],
        target: 'map',
        view: view
      });
...
//$(".layer-switcher div li label:contains('Векторный слой')").addClass('nonactivepanel');
...
//Обработчки движения мышки
      ...
      var pointerMoveHandler = function(evt) {        
        ...
        //добавить флаг для рисования и скрыть при рисовании:
        changecolor(pixel);
//        if (flag==1){
//        helpMsg='';
//        //helpMsg="Click to get features";
//        }
//        else{
...
////////////////////////////////////////////////// 
////Оверлей для выбранных фиичеров       
...
////Невидимый стиль
    ...
    // Interacton "Click"
      var selectClick = new ol.interaction.Select({
        ...
        layers: [user_layer, archive_layer, newlayer],
        multi: true,
        style: new ol.style.Style({
          stroke: new ol.style.Stroke({
            color: '#f00',
...
//Активация кнопок поиска снимков  
    $("#datetimepicker6,#datetimepicker7").on("dp.change", function (e) {
        ...
        if (empty==false && endofdraw==true) {
            $('#findbtn').removeClass('disabled'); // updated according to http://stackoverflow.com/questions/7637790/how-to-remove-disabled-attribute-with-jquery-ie
        }
        else {
            $('#findbtn').addClass('disabled'); // updated according to http://stackoverflow.com/questions/7637790/how-to-remove-disabled-attribute-with-jquery-ie
        }};

//////////////////////////////////////////////////
//Выделение слоев при выборе строки в таблице
...
//Отобразить найденные снимки в таблице   
    $("#archivetbl").empty();
    var table = $("<table id='arctbl'></table>").addClass('table hover table-striped table-bordered');
    var row = $('<thead><tr><th>Номер</th><th>ID</th><th>Имя снимка</th><th>Дата</th></tr></thead><tbody>');
    table.append(row);
    ...
    for (var i=0;i<number;i++){
        var num = data[i]["id"];
        var activefeature = archive_src.getFeatureById('archive.'+num); // ПРОБЛЕМА!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        newlayer.getSource().addFeature(activefeature);
        var row = $("<tr><td>"+(i+1)+"</td><td id='colid'>"+num+"</td><td>"+activefeature.get('imgname')+"</td><td>"+activefeature.get('time')+"</td><td><a data-toggle='modal' id='infobtn' data-target-id='1' data-target='#modalinfo'><i class='fa fa-shopping-cart'></i></a><a data-toggle='modal' id='infobtn' data-target-id='1' data-target='#modalinfo'><i class='fa fa-eye'></i></a></td></tr>");
        table.append(row);
        }
    var row = $('</tbody>');
    table.append(row);
...
function refreshpanel(activepanel) {
//closeNav();
